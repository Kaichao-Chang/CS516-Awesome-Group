...
class ProductReview:
    def __init__(
            self, user_id: int, user_name: str, product_id: int, product_name: str, content: str,
            star: int, created_at: str):
    ...
    def get_reviews_of_one_user(user_id: int):
        rows = app.db.execute(
            "SELECT uid, CONCAT(firstname, ' ', lastname) AS user_name, pid, name, content, star, created_at "
            "FROM ProductReviews "
            "LEFT JOIN Users ON ProductReviews.uid = Users.id "
...
class SellerReview:
    def __init__(
            self, user_id: int, user_name: str, seller_id: int, seller_name: str, content: str,
            star: int, created_at: str):
        self.user_id = user_id
        self.user_name = user_name
    ...
    def get_reviews_of_one_user(user_id: int):
        rows = app.db.execute(
            "WITH SRUser(customer_id, user_name, seller_id, content, star, created_at) AS "
            "(SELECT customer_id, CONCAT(Users.firstname, ' ', Users.lastname) AS user_name, seller_id, content, star, created_at "
            "FROM SellerReviews "
            "LEFT JOIN Users ON SellerReviews.customer_id = Users.id "
            "WHERE SellerReviews.customer_id = :user_id) "
            "    SELECT customer_id, user_name, seller_id, CONCAT(Users.firstname, ' ', Users.lastname) AS seller_name, content, star, created_at "
            "    FROM SRUser "
